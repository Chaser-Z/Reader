...
class ReadChapterListModel: NSObject,NSCoding {
    ...
    // MARK: -- 操作
    func readChapterModel(isUpdateFont:Bool = false) ->ReadChapterModel? {
        
        if ReadChapterModel.IsExistReadChapterModel(bookID: bookID, chapterID: id) {
            
            return ReadChapterModel.readChapterModel(bookID: bookID, chapterID: id, isUpdateFont: isUpdateFont)
        }
    ...
    /// 获得阅读内容模型
    func readChapterModel(readRecordModel:ReadRecordModel) ->ReadChapterModel {
    ...
    /// 清理
    func clearPageCount(readRecordModel:ReadRecordModel) {
