...
class ZHNBottomView: ZHNBaseView,ASValueTrackingSliderDelegate {
    ...
    override func addSubviews() {
        ...
        for i in 0 ..< funcIcons.count {
            ...
            button.tag = i
            button.addTarget(self, action: #selector(ZHNBottomView.clickButton(_:)), for: UIControlEvents.touchUpInside)
            addSubview(button)
        }
        
        // 上一章
        previousChapter = UIButton(type:.custom)
        previousChapter.titleLabel?.font = Font_12
        ...
        previousChapter.addTarget(self, action: #selector(ZHNBottomView.clickPreviousChapter), for: .touchUpInside)
        addSubview(previousChapter)
        
        // 下一章
        nextChapter = UIButton(type:.custom)
        nextChapter.titleLabel?.font = Font_12
        ...
        nextChapter.addTarget(self, action: #selector(ZHNBottomView.clickNextChapter), for: .touchUpInside)
...
//        }
    ...
    override func layoutSubviews() {
        ...
        // 上一章
        previousChapter.frame = CGRect(x: 0, y: Space_4, width: 55, height: 32)
        
        // 下一章
        nextChapter.frame = CGRect(x: width - previousChapter.width, y: Space_4, width: previousChapter.width, height: previousChapter.height)
        
        // 进度条
        let sliderX = previousChapter.frame.maxX + Space_4
        let sliderW:CGFloat = width - 2 * sliderX
        slider.frame = CGRect(x: sliderX, y: Space_4, width: sliderW, height: previousChapter.height)
