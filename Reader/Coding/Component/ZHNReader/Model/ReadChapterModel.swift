...
class ReadChapterModel: NSObject,NSCoding {
    ...
    func updateFont(isSave:Bool = false) {
        ...
        let readAttribute = ReadConfigure.shared().readAttribute()
        
        if self.readAttribute != readAttribute {
            
            self.readAttribute = readAttribute
            
            rangeArray = ReadParser.ParserPageRange(string: content, rect: GetReadViewFrame(), attrs: readAttribute)
    ...
    class func readChapterModel(bookID:String, chapterID:String, isUpdateFont:Bool = false) ->ReadChapterModel {
        
        var readChapterModel:ReadChapterModel!
        
        if ReadChapterModel.IsExistReadChapterModel(bookID: bookID, chapterID: chapterID) { // 存在
            
            readChapterModel = ReadKeyedUnarchiver(folderName: bookID, fileName: chapterID) as! ReadChapterModel
            
            if isUpdateFont {readChapterModel.updateFont(isSave: true)}
            
        }else{ // 不存在
            
            readChapterModel = ReadChapterModel()
