        ...
        if (fabs(tempPoint.x) > 0.01) { // 滚动有值了
            ...
            if (self.openAnimate && self.isPan) {
                ...
                if (self.tempController) {
                    ...
                    if (self.isLeft) {
#pragma mark   修改 touchPoint - > tempPoint
                        self.tempController.view.frame = CGRectMake(tempPoint.x - ViewWidth, 0, ViewWidth, ViewHeight);
...
#pragma mark - 17.3.7 修改
        // 代理回调
        if ([self.delegate respondsToSelector:@selector(coverController:currentController:finish:)]) {
            
            [self.delegate coverController:self currentController:self.currentController finish:YES];
        }
    }
}
