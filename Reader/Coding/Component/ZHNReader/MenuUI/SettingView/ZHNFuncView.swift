...
/// 小说FuncView支持类型
enum ZHNFuncViewType:NSInteger {
    case effect
    case font
    case fontSize
}
...
class ZHNFuncView: ZHNBaseView {
    ...
    /// 类型
    private(set) var funcType: ZHNFuncViewType!
    ...
    /// 初始化方法
    convenience init(frame:CGRect,readMenu: ZHNReadMenu,funcType:ZHNFuncViewType,title:String) {
        
        self.init(frame:frame,readMenu:readMenu,funcType:funcType,title:title,labels:[],selectIndex:0)
    }
    
    /// 初始化方法
    init(frame:CGRect,readMenu: ZHNReadMenu,funcType: ZHNFuncViewType,title:String,labels:[String],selectIndex:NSInteger) {
    ...
    override func addSubviews() {
        ...
        if funcType == .font || funcType == .effect {
            ...
            for i in 0..<count {
                ...
                button.addTarget(self, action: #selector(ZHNFuncView.clickButton(button:)), for: .touchUpInside)
            ...
            leftButton.addTarget(self, action: #selector(ZHNFuncView.clickFontSize(button:)), for: .touchUpInside)
            ...
            rightButton.addTarget(self, action: #selector(ZHNFuncView.clickFontSize(button:)), for: .touchUpInside)
    ...
    func clickFontSize(button:UIButton) {
        ...
        if button.tag == 0 { // left
            ...
            // 没有小于最小字体
            if (ZHNReadConfigure.shared().fontSize - fontSpace) >= ReadMinFontSize {
                
                ZHNReadConfigure.shared().fontSize -= fontSpace
                
                readMenu.delegate?.readMenuClickSetuptFontSize?(readMenu: readMenu, fontSize: CGFloat(ZHNReadConfigure.shared().fontSize))
            }
            
        }else{ // right
            
            // 没有大于最大字体
            if (ZHNReadConfigure.shared().fontSize + fontSpace) <= ReadMaxFontSize {
