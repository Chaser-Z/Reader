...
class ZHNReadView: UIView {
    ...
    /// 左光标
    private var LCursorView: ZHNCursorView!
    
    /// 右光标
    private var RCursorView: ZHNCursorView!
    ...
    @objc private func longAction(long:UILongPressGestureRecognizer) {
        ...
        // 禁止上下滚动使用
        if ZHNReadConfigure.shared().effectType == ZHNEffectType.upAndDown.rawValue { return }
        ...
        if long.state == .began {
            ...
            // 发送通知
            ZHNReadView.PostNotification(userInfo: [Key_ReadView_Ges_isOpen: NSNumber(value: true)])
            ...
            // 获得选中区域
            selectRange = ZHNReadAuxiliary.GetTouchLineRange(point: point, frameRef: frameRef)
            
            // 获得选中选中范围
            rects = ZHNReadAuxiliary.GetRangeRects(range: selectRange!, frameRef: frameRef, content: content)
            ...
            if !rects.isEmpty {
                ...
                // 发送通知
                ZHNReadView.PostNotification(userInfo: [Key_ReadView_Ges_isOpen: NSNumber(value: false)])
            }
        }
    }
    ...
    /// 拖拽事件解析
    func drag(status:ZHNPanStatus, point:CGPoint, windowPoint:CGPoint) {
        ...
        if status == .begin {
            ...
            if LCursorView.frame.insetBy(dx: ZHNCursorOffset, dy: ZHNCursorOffset).contains(point) { // 触摸到左边光标
                isLorR_Cursor = true
                
                isTouchCursor = true
                
            }else if RCursorView.frame.insetBy(dx: ZHNCursorOffset, dy: ZHNCursorOffset).contains(point) { // 触摸到右边光标
                isLorR_Cursor = false
            ...
            if isTouchCursor && selectRange != nil {
                ...
                // 触摸到的位置
                let location = ZHNReadAuxiliary.GetTouchLocation(point: point, frameRef: frameRef)
    ...
    private func cursor(isShow:Bool) {
        ...
        if isShow {
            ...
            if !rects.isEmpty && LCursorView == nil {
                LCursorView = ZHNCursorView()
                LCursorView.isTorB = true
                addSubview(LCursorView)
                
                RCursorView = ZHNCursorView()
    ...
    override func draw(_ rect: CGRect) {
        ...
        if selectRange != nil && !rects.isEmpty {
            ...
            let path = CGMutablePath()
             // 选中区域颜色
             UIColor(red:0.79, green:0.85, blue:0.90, alpha:1.00).withAlphaComponent(0.5).setFill()
    ...
    private func menu(isShow:Bool) {
        ...
        if isShow { // 显示
            if !rects.isEmpty {
                let rect = ZHNReadAuxiliary.GetMenuRect(rects: rects, viewFrame: bounds)
    ...
    class func RegisterNotification(observer:Any, selector:Selector) {
        ...
        NotificationCenter.default.addObserver(observer, selector: selector, name: NSNotification.Name(rawValue: ZHNNotificationName_ReadView_Ges), object: nil)
    }
    
    /// 移除通知
    class func RemoveNotification(observer:Any) {
        
        NotificationCenter.default.removeObserver(observer, name: NSNotification.Name(rawValue: ZHNNotificationName_ReadView_Ges), object: nil)
    }
    
    /// 发送通知
    class func PostNotification(userInfo:[AnyHashable : Any]?) {
        
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: ZHNNotificationName_ReadView_Ges), object: nil, userInfo: userInfo)
    }
