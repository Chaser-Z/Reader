...
@objc protocol ReadMenuDelegate:NSObjectProtocol {
    ...
    /// 状态栏 将要 - 隐藏以及显示状态改变
    @objc optional func readMenuWillShowOrHidden(readMenu:ReadMenu, isShow:Bool)
    
    /// 状态栏 完成 - 隐藏以及显示状态改变
    @objc optional func readMenuDidShowOrHidden(readMenu:ReadMenu, isShow:Bool)
    
    /// 点击下载
    @objc optional func readMenuClickDownload(readMenu:ReadMenu)
    
    /// 点击书签按钮
    @objc optional func readMenuClickMarkButton(readMenu:ReadMenu, button:UIButton)
    
    /// 点击上一章（上一话）
    @objc optional func readMenuClickPreviousChapter(readMenu:ReadMenu)
    
    /// 点击下一章（下一话）
    @objc optional func readMenuClickNextChapter(readMenu:ReadMenu)
    
    /// 停止滚动进度条
    @objc optional func readMenuSliderEndScroll(readMenu:ReadMenu,slider:ASValueTrackingSlider)
    
    /// 点击背景颜色
    @objc optional func readMenuClickSetuptColor(readMenu:ReadMenu,index:NSInteger,color:UIColor)
    
    /// 点击翻书动画
    @objc optional func readMenuClickSetuptEffect(readMenu:ReadMenu,index:NSInteger)
    
    /// 点击字体
    @objc optional func readMenuClickSetuptFont(readMenu:ReadMenu,index:NSInteger)
    
    /// 点击字体大小
    @objc optional func readMenuClickSetuptFontSize(readMenu:ReadMenu,fontSize:CGFloat)
    
    /// 点击日间夜间
    @objc optional func readMenuClickLightButton(readMenu:ReadMenu,isDay:Bool)
    
    /// 点击章节列表
    @objc optional func readMenuClickChapterList(readMenu:ReadMenu,readChapterListModel:ReadChapterListModel)
    
    /// 点击书签列表
    @objc optional func readMenuClickMarkList(readMenu:ReadMenu,readMarkModel:ReadMarkModel)
...
class ReadMenu: NSObject ,UIGestureRecognizerDelegate{
    ...
    /// LeftView
    private(set) var leftView: RMLeftView!
    ...
    /// 亮度
    private(set) var lightView: RMLightView!
    
    /// 遮盖亮度
    private var coverView:UIView!
    
    /// 亮度按钮
    private var lightButton:HaloButton!
    
    /// 小说阅读设置
    private var novelsSettingView:RMSettingView!
    ...
    /// 初始化
    class func readMenu(vc:ReadController,delegate:ReadMenuDelegate) ->ReadMenu {
        
        let readMenu = ReadMenu(vc:vc,delegate:delegate)
        
        return readMenu
    }
    
    /// 初始化函数
    private init(vc:ReadController,delegate:ReadMenuDelegate) {
    ...
    private func creatUI() {
        ...
        // 设置为日夜间 默认日间
        lightButton.isSelected = MyUserDefaults.boolForKey(Key_IsNighOrtDay)
    }
    ...
    private func initTapGestureRecognizer() {
        ...
        // 单击手势
        singleTap = UITapGestureRecognizer(target: self, action: #selector(ReadMenu.touchSingleTap))
    ...
    /// 点击这些控件不需要执行手势
    private let ClassString:[String] = ["ASValueTrackingSlider","UITableViewCellContentView","UISlider","Reader.RMLeftView","Reader.RMTopView","Reader.RMBottomView","Reader.HaloButton","Reader.RMLightView","Reader.RMColorView","Reader.RMFuncView","Reader.RMSettingView"]
    ...
    private func initLeftView() {
        ...
        leftView = RMLeftView(frame:CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight),readMenu:self)
        ...
        leftView.addTarget(self, action: #selector(ReadMenu.clickLeftView), for: .touchUpInside)
    ...
    private func initNovelsSettingView() {
        ...
        novelsSettingView = RMSettingView(frame:CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: NovelsSettingViewH),readMenu:self)
    ...
    private func initCoverView() {
        ...
        coverView.alpha = MyUserDefaults.boolForKey(Key_IsNighOrtDay) ? 1.0 : 0
    ...
    private func initLightView() {
        ...
        lightView = RMLightView(readMenu:self)
    ...
    private func initLightButton() {
        ...
        lightButton = HaloButton(CGRect(x: ScreenWidth - lightButtonWH - Space_1, y: ScreenHeight, width: lightButtonWH, height: lightButtonWH), haloColor:UIColor.black.withAlphaComponent(0.75))
        ...
        lightButton.addTarget(self, action: #selector(ReadMenu.clickLightButton), for: .touchUpInside)
    }
    ...
    func clickLightButton(button:UIButton) {
        ...
        MyUserDefaults.setBool(button.isSelected, key: Key_IsNighOrtDay)
    ...
    private func initTopView() {
        ...
        topView.back.addTarget(self, action: #selector(ReadMenu.clickBack), for: .touchUpInside)
    }
