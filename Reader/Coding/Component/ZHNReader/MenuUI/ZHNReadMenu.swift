...
@objc protocol ZHNReadMenuDelegate:NSObjectProtocol {
    
    /// 状态栏 将要 - 隐藏以及显示状态改变
    @objc optional func readMenuWillShowOrHidden(readMenu:ZHNReadMenu, isShow:Bool)
    
    /// 状态栏 完成 - 隐藏以及显示状态改变
    @objc optional func readMenuDidShowOrHidden(readMenu:ZHNReadMenu, isShow:Bool)
    
    /// 点击下载
    @objc optional func readMenuClickDownload(readMenu:ZHNReadMenu)
    
    /// 点击书签按钮
    @objc optional func readMenuClickMarkButton(readMenu:ZHNReadMenu, button:UIButton)
    
    /// 点击上一章（上一话）
    @objc optional func readMenuClickPreviousChapter(readMenu:ZHNReadMenu)
    
    /// 点击下一章（下一话）
    @objc optional func readMenuClickNextChapter(readMenu:ZHNReadMenu)
    
    /// 停止滚动进度条
    @objc optional func readMenuSliderEndScroll(readMenu:ZHNReadMenu,slider:ASValueTrackingSlider)
    
    /// 点击背景颜色
    @objc optional func readMenuClickSetuptColor(readMenu:ZHNReadMenu,index:NSInteger,color:UIColor)
    
    /// 点击翻书动画
    @objc optional func readMenuClickSetuptEffect(readMenu:ZHNReadMenu,index:NSInteger)
    
    /// 点击字体
    @objc optional func readMenuClickSetuptFont(readMenu:ZHNReadMenu,index:NSInteger)
    
    /// 点击字体大小
    @objc optional func readMenuClickSetuptFontSize(readMenu:ZHNReadMenu,fontSize:CGFloat)
...
class ZHNReadMenu: NSObject ,UIGestureRecognizerDelegate{

    /// 控制器
    private(set) weak var vc:ZHNReadController!
    
    /// 代理
    private(set) weak var delegate: ZHNReadMenuDelegate!
    ...
    class func readMenu(vc: ZHNReadController,delegate: ZHNReadMenuDelegate) ->ZHNReadMenu {
        ...
        let readMenu = ZHNReadMenu(vc:vc,delegate:delegate)
        
        return readMenu
    }
    
    /// 初始化函数
    private init(vc:ZHNReadController,delegate: ZHNReadMenuDelegate) {
    ...
    private func initTapGestureRecognizer() {
        ...
        // 单击手势
        singleTap = UITapGestureRecognizer(target: self, action: #selector(ZHNReadMenu.touchSingleTap))
    ...
    private func initLeftView() {
        ...
        leftView = ZHNLeftView(frame:CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight),readMenu:self)
        ...
        leftView.addTarget(self, action: #selector(ZHNReadMenu.clickLeftView), for: .touchUpInside)
